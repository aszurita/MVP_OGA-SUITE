...
// Estrategia de Datos
  ...
  if(!tieneAcceso() && !window.location.pathname.includes("SinAcceso.aspx") && restringidos.includes(currentPage) ){
      return window.location.replace("http://vamos.bancoguayaquil.com/sitios/informaciongerencial/OGA_Suite/Desarrollo/SinAcceso.aspx")
  }
  let html ='<li>\
  ...
  var bloque_seguridad_autorizaciones = '\
                        <a href="http://vamos.bancoguayaquil.com/sitios/informaciongerencial/OGA_Suite/Produccion/Autorizacion.aspx" class="icon-menu-item quicklinks">\
  ...
  let nav_horizontal = '<div class="icon-container position-relative d-none d-sm-inline-flex">\
                    ...
                    <div class="position-relative d-none d-sm-inline-block">\
                      ...
                      <div class="dropdown-menu dropdown-menu-right mt-3  position-absolute" id="iconMenuDropdown" style="height: 430px;">\
                        ...
                        <a href="http://vamos.bancoguayaquil.com/sitios/informaciongerencial/OGA_Suite/Produccion/Entidades_Pendientes.aspx" class="icon-menu-item quicklinks">\
                        <i class="iconsminds-file-edit" style="font-size:50px;color:#D2006E"></i>\
                        <span style ="padding:1px;">Entidades Pendientes</span>\
                        </a>\
                        <a href="http://vamos.bancoguayaquil.com/sitios/informaciongerencial/OGA_Suite/Produccion/Nomenclaturas_campañas.aspx" class="icon-menu-item quicklinks">\
...
function getTerminosAll(){
  ...
  return {t_all, atributos, terminos, datos_personales, golden, cde, ar}
}
...
function estandarizarSegmentacion(){
  ...
  segmentador.nextSibling.nextSibling.innerHTML = `
  ...
  <a class="dropdown-item" onclick="segmentar('(AR) Atributo de Referencia')">(AR) Atributo de Referencia</a>\
  <a class="dropdown-item" onclick="segmentar('(CDE) Elemento clave de datos')">(CDE) Elemento clave de datos</a>\
...
function segmentar(criterio){
  let criterios = ["(AR) Atributo de Referencia", '(CDE) Elemento clave de datos']
  let parsedCriterio = criterio
  if(!criterios.includes(criterio)) parsedCriterio = mayusc_each_word(criterio)
  document.getElementById("segmentar-btn").innerText = parsedCriterio;
  ...
  }else if(criterio === "(AR) Atributo de Referencia"){
    window.glosario.t_all = sortGlosario(window.glosario.ar)
  }else if(criterio === "(CDE) Elemento clave de datos"){
    window.glosario.t_all = sortGlosario(window.glosario.cde)
  }else{
...
function toggleArrow(elemento){
  ...
  let cardResumen = $(".resumen-metadatos-linaje")
  if(cardResumen.length !== 0 && !cardResumen.hasClass("shown")){
    cardResumen.toggleClass("shown")
  }
...
async function fichaAtributo(){
  ...
  if(pertenece==="false"){
    ...
    $(".resumen-metadatos-linaje-button").on("click", (function(t) {
      ...
      $(".simple-icon-trophy").toggleClass("text-primary")
    }
    ))
...
async function fillModalCalidad(){
  ...
  const resultado = window.entidadesMetadatosLinaje.filter(obj => {
    if (!entidadesUnicas.has(obj.complemento)) {
      entidadesUnicas.add(obj.complemento);
      return true;
    }
    return false;
  });
  ...
  $("#entidad-search").autocomplete({
    ...
    select: function(event, ui){
      ...
      $("#entidad-search").prop('disabled', true)
...
function filtrarTablaCalidad(){
  ...
  let filtrado = []
  Object.entries(tablas).forEach(([key, value]) => {if(key.includes(window.entidad_actual)) filtrado.push(value)})
  let data_tabla=[];
...
function fillInfoEntidadIndicadores(entidad){
  let selectedItem = window.calidadTablasOficiales.find(item => `[${item.txt_host}].[${item.txt_fuente_esquema}].[${item.txt_desc_tabla}]` === `[${entidad.txt_host}].[${entidad.txt_fuente_esquema}].[${entidad.txt_desc_tabla}]`) || window.entidadesMetadatosLinaje.find(item => `[${item.base}].[${item.esquema}].[${item.tabla}]` === `[${entidad.base}].[${entidad.esquema}].[${entidad.tabla}]`);
  if (selectedItem) {
...
function fillListaEntidades(){
  ...
  entidadesCalidad.forEach(item => {
    ...
    window.listaEntidadesCalidad.push(item.tabla)
...
function agruparYContar(lista) {
  const resultado = [];

  // Agrupar usando un Map
  const grupos = new Map();

  lista.forEach(item => {
      const { servidor, base, esquema, tabla, campo } = item;

      // Asegurarse de que el servidor esté en el grupo
      if (!grupos.has(servidor)) {
          grupos.set(servidor, {
              bases: new Set(),
              esquemas: new Set(),
              tablas: new Set(),
      ...
      const grupo = grupos.get(servidor);
      
      // Agregar valores únicos a los conjuntos
      grupo.bases.add(base);    // Agregar base única
      grupo.esquemas.add(esquema);  // Agregar esquema único
      grupo.tablas.add(tabla);      // Agregar tabla única
      grupo.campos.push(campo);      // Agregar campo 
  });

  // Preparar el resultado
  grupos.forEach((data, servidor) => {
      resultado.push({
          servidor: servidor,
          bases: data.bases.size,     // Contar bases únicas
          esquemas: data.esquemas.size, // Contar esquemas únicos
          tablas: data.tablas.size,       // Contar tablas únicas
          campos: data.campos.length        // Contar campos 
      });
  });
...
//Buscar elementos mediante regex
function buscarElementos(search_value, list){
  if(search_value.includes("(")) search_value = search_value.replaceAll("(","\\(");
  if(search_value.includes(")")) search_value = search_value.replaceAll(")","\\)");
  let regex = new RegExp(search_value,"i");
  var results = list.filter(item => regex.test(item.tabla));
  return results;
}
...
function deleteFiltroCalidad(){
  ...
  redrawDataTable("indicadores_calidad", data_tabla)
  $('.btnGuardarIndicadores').prop('disabled', true);
  $('#entidad-search').prop('disabled', false);
}
...
function Proceso_Autorizacion_Aprobar(data,dataactual) {
      ...
      if (window.tabla_autorizacion[llavetabla] !== undefined) {
...
//Tabla autorizaciones
  ...
  function showNotification(placementFrom, placementAlign, type, message) {
    $.notify(
      {
        title: "Notification",
        message: message
...
function escribirListaCorreo(SOLICITADO,USUARIO,TIPO_CAMBIO, MENSAJE){
  ...
  //Funcion para cargar top aportes
  function setTopAportes(){
    ...
    leaderboardUsers.forEach((user, index) => {
      ...
      userList += `<div class="${user.position === 1? "text-white" : ""} d-flex justify-content-between" style="gap:12px;padding: 0px 8px; ${user.position === 1 ? "background-color:#245794;": ""}" >\
  ...
  function getAporteLeaderboard(){
    let catalogos = getCatalogoOGASUITE("2");
    let data =[]
    const mesAnteriorTexto = new Date(new Date().setMonth(new Date().getMonth() - 1))
      .toISOString()
      .slice(0, 7);
    $().SPServices({
      operation: "GetListItems",
      ...
      CAMLQuery: `<Query>\
                    <Where>\
                      <And>\
                        ...
                        <BeginsWith>\
                          <FieldRef Name="FECHA_APROBACION" />\
                          <Value Type="Text">${mesAnteriorTexto}</Value>\
                        </BeginsWith>\
                      </And>\
                    </Where>\
                  </Query>`,
    ...
    const usuariosExcluidos = ["ccuenca", "jsalas", "mzambrano10"]
    ...
    $("#top-aporte").text(aporteUsuarios[window.CodigoEmpleado[window.current_email]] || "--")
  ...
  function setDataTableEntidades_pendientes(data_tabla){
    ...
    let selectTemplate = crearSelectDataOwner();
    ...
    let selectDataSteward = crearSelectDataSteward()
    $(selectDataSteward).select2({
      theme: "bootstrap",
      placeholder: "",
      maximumSelectionSize: 6,
      width: '100%'
    ...
    var table = $('#tbl_entidades_pendientes').DataTable({
      ...
      columns: [
          ...
          { data: null,//campo de tablas
            render: function (data,type,row){
              // return "<p onclick=''>["+data.base+"] ."+"["+data.esquema+"] ."+"["+data.tabla+"]</p>"
  ...
  function crearSelectDataOwner() {
    let select = document.createElement('select');
    select.classList.add('form-control', 'select2-single', 'dataownerselect');
    select.setAttribute('aria-hidden', 'true');
    select.setAttribute('data-width', '100%');

    
    // Añadir las opciones una sola vez
    $.each(window.empleados_entidades_pendientes, function(value, text) {
      select.append(new Option(value, text));
    });
    
    return select;
  }

  function crearSelectDataSteward(){
    let select = document.createElement('select');
    select.classList.add('form-control', 'select2-single', 'dataselect');
    select.setAttribute('aria-hidden', 'true');
    select.setAttribute('data-width', '100%');
  ...
  function ListenerEntidadesPendientes() {
    ...
    $('#tbl_entidades_pendientes tbody').on('click', '.btnGuardarRegistro', function() {
      ...
      if (codigoowner.trim() !== "" || codigosteward.trim() !== "" || clasificacion.trim() !== "" || descripción_tabla.trim() !== "") {
        ...
        if (codigoowner !== dataownerActual && codigoowner.trim() !== "" && codigoowner !== "&nbsp;") {
        ...
        if (codigosteward !== datastewardActual && codigosteward.trim() !== "" && codigosteward !== undefined && codigosteward !== "&nbsp;") {
        ...
        if (clasificacion !== data.clasificacion && clasificacion.trim() !== "") {
        ...
        if (descripciónChanged = descripción_tabla !== '' && descripción_tabla !== undefined && descripción_tabla !== data.descripcion) {
  ...
  function segmentarTablaEntidadesDocumentadas(tabla, valor) {
    ...
    if (valor.toLocaleLowerCase() === "todos los usuarios".toLocaleLowerCase()) {
        ...
        if (valor.toLocaleLowerCase() === "mis pendientes".toLocaleLowerCase() ) {
          ...
          // tabla.columns(3).search(window.current_user).draw();
  ...
  function actualizarIndicadores(id_atributo) {
    ...
    let promedioTexto = promedio === 0 ? "N/D" : (Math.floor(promedio * 100)/100).toFixed(2) + "%";
    window.promedioAtributo = promedio
    document.getElementById("numeroEntidadesPromedio").innerHTML =  `${numeroEntidades} ${numeroEntidades === 1 ? "entidad evaluada":"entidades evaluadas"} `
  ...
  function setDataTableAbrev_campañas(data_tabla){
    ...
    table.column(4).search(window.current_user).draw();
  }

...
def get_campos(
    ...
    # búsqueda parcial en nombre de tabla (LIKE) — útil al agrupar por campos desde vista tabla
    tabla_q: Optional[str] = Query(None, description="Búsqueda parcial (LIKE) en nombre de tabla"),
    # paginación
    ...
    **Columnas clave devueltas:**
    ...
    if tabla:
        ...
        params.append(tabla)
    if tabla_q:
        # LIKE parcial en nombre de tabla — muestra todos los campos de tablas que coincidan
        where.append("UPPER(tabla) LIKE ?")
        params.append(f"%{tabla_q.upper()}%")
    if plataforma:
